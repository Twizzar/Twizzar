...
[TestFixture]
public class ProjectNameQueryTests
{
    #region members
    ...
    public async Task When_FixtureItemConfigurationStartedEvent_is_in_eventStore_return_the_span_of_the_event()
    {
        ...
        var sut = new ProjectNameQuery(eventStore.Object);

        // act
        var result = await sut.GetProjectNameAsync(RandomString());

        // assert
        var projectName = AssertAndUnwrapSuccess(result);
        projectName.Should().Be(e.ProjectName);
    }
