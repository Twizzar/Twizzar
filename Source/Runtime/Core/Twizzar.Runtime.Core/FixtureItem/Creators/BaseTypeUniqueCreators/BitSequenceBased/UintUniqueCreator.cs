...
{
    /// <summary>
    /// The generator for the <see cref="uint"/> type using bit sequence based generator <see cref="BitSequenceBasedUniqueCreator{T}"/>.
    /// </summary>
    public class UintUniqueCreator : BitSequenceBasedUniqueCreator<uint>
    {
        #region static fields and constants

        private static byte[] _uniqueValue = new byte[sizeof(uint)];
        private static int _currentPartitionIndex;
        private static BitArray[] _partitions = new BitArray[sizeof(uint)];
        ...
        /// <inheritdoc />
        protected override int TotalBits => sizeof(uint) * BitsOfByte;

        /// <inheritdoc />
        protected override int PartitionSize => sizeof(uint);
        ...
        /// <inheritdoc />
        protected override uint ConvertByteArrayToT()
        {
            try
            {
                var result = BitConverter.ToUInt32(this.UniqueValue, 0);
